...
function WhatsApp() {
 var ph = '';
 if ($('#whatsapp .name').val() == '') { // Cek Nama
  ph = $('#whatsapp .name').attr('placeholder');
  alert('Please write ✍️ ' + ph);
  $('#whatsapp .name').focus();
  return false;
  ...
  return false;
 } else if (reg.test($('#whatsapp .email').val()) == false) { //Email validation
  alert('Invalid E-mail address.');
  $('#whatsapp .email').focus();
  return false;
 } else if ($('#whatsapp .sendto').val() == '') { // Checking User-Inp
  ph = $('#whatsapp .sendto').attr('placeholder');
  alert('Please write ✍️ ' + ph);
  $('#whatsapp .sendto').focus();
  return false;
 } else {
  if (!confirm("Have you installed WhatsApp?")) {
   window.open("https://www.whatsapp.com/download/");
  } else {
   // Checking Devices (Mobile/Desktop)
   var url_wa = 'https://web.whatsapp.com/send';
   if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
    url_wa = 'whatsapp://send/';
   }
   // Getting values
   var sendto = $('#whatsapp .sendto').val(),
     via_url = location.href,
     name = $('#whatsapp .name').val(),
     email = $('#whatsapp .email').val(),
     chat = $('#whatsapp .chat').val();
   $(this).attr('href', url_wa + '?phone=91 ' + sendto + '&text=Hello OpenAdmits, I am ' + name + '%0AMy email is ' + email + '%0A%0AQuery: ' + chat + '%0A%0AI sended this from your website: ' + via_url);
   var w = 960,
     h = 540,
